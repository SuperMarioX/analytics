...
@EasyService
public class DataExportRequestManager implements IDataExportRequestManager{
	...
	public CADataMatrix locateMatrix(DataExportRequest request) {
		...
		for (CAReport rp : reports) {
			...
			List<CADataMatrix> mlist = rp
					.matrixContainsAllDimension(new ArrayList<String>(allCol));
	...
	public StringBuilder getSelectSql(DataExportRequest request,
			CADataMatrix cadm) {
		...
		for (String dim : request.getDimensions()) {
			if (request.getTimeScale().equals(
					DataExportRequest.TIME_SCALE_1MONTH)) {
				if (dim.equals("topd") || dim.equals("topwd")) {
					continue;
				}
			}
			if (request.getTimeScale().equals(
					DataExportRequest.TIME_SCALE_1YEAR)) {
				if (dim.equals("topd") || dim.equals("topwd")
						|| dim.equals("topm")) {
					continue;
				}
		...
		for (String m : metrics) {
			...
			if (groups != null && !groups.isEmpty()) {
				sqlSelect.append("sum(").append(mc.getColName())
						.append(") AS ").append(m).append(",");
				orderReplacement.put(mc.getColName(), m);
			} else {
				sqlSelect.append(mc.getColName()).append(" AS ").append(m)
						.append(",");
			}
		}
		if (groups != null && !groups.isEmpty()) {
	...
	private void appendCondition(CADataMatrix cadm, DataExportRequest request,
			StringBuilder sqlSelect) {
		...
		sqlSelect.append("WHERE ").append(CADataMatrix.DATE_COLUMN_NAME)
				.append(" >= ").append(start);
		sqlSelect.append(" AND ").append(CADataMatrix.DATE_COLUMN_NAME)
				.append(" <= ").append(end);
		if (request.getFilter() == null
				|| "".equals(request.getFilter().trim())) {
			return;
		} else {
			/**
			 * 仅支持= > < <> >= <= like
	...
	protected static String parseFilter(CADataMatrix cadm,
			DataExportRequest request) {
		String customWhere = request.getFilter().replaceAll(
				"\\s+[a|A][n|N][d|D]\\s+", " and ");
		...
		for (String aWhere : whereSeg) {
			...
			sb.append(" ").append(colName).append(" ").append(aWhere)
					.append(" AND");
			hasAnd = true;
		}
		String newWhere = "";
	...
	private void appendSort(CADataMatrix cadm, DataExportRequest request,
			StringBuilder sqlSelect, Map<String, String> orderReplacement) {
	...
	private void appendLimit(CADataMatrix cadm, DataExportRequest request,
			StringBuilder sqlSelect) {
	...
	protected void activate(ComponentContext ctx) {
		String carecords = confAdmin
				.getFullPathRelatedToConfRoot("carecords.xml");
		...
		try {
			...
			throw new ReportException("启动DataExportRequestManager失败。", e);
		} finally {
			if (in != null) {
				try {
					in.close();
		...
		config = confAdmin.get(CAReportServerConf.class.getSimpleName(),
				CAReportServerConf.class);
		CALogServerConf logConf = confAdmin.get(
				CALogServerConf.class.getSimpleName(), CALogServerConf.class);
		if (config == null) {
		...
		log.info("启动DataExportRequestManager成功");
	}
}
