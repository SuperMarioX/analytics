...
(function ($) {
  var DatePicker = function () {
    var ids = {},
      ...
      layout = function(el) {
        ...
        var height = 1.13*tbl.offsetHeight;
        cal.css({
      ...
      click = function(ev) {
        ...
        var el = $(ev.target);
        if (el.is('a')) {
        	if(el.css('color') == "rgb(221, 221, 221)") {
        		return false;
        	}
          ev.target.blur();
          ...
          if (parentEl.is('th')) {
            ...
            if (el.hasClass('datepickerMonth')) {
              ...
              if(options.mode == 'range') {
               ...
               // rangeDatePicker.setStartDate(options.date[0]);
               // rangeDatePicker.setEndDate(options.date[1]);
              ...
              // clicked either next/previous arrows
            	if(options.current.getFullYear() == 2005 && options.current.getMonth() == 1 && el.hasClass('datepickerGoPrev')) {
            		return false;
            	}
            	else if(el.hasClass('datepickerGoNext') && (options.current.getFullYear() > new Date().getFullYear() || (options.current.getFullYear() == new Date().getFullYear() && 
            			options.current.getMonth() >= new Date().getMonth()))) {
            		return false;
            	}
              if(tblEl.eq(0).hasClass('datepickerViewDays')) {
      ...
      init: function(options){
        ...
        return this.each(function(){
          if (!$(this).data('datepicker')) {
            ...
            var strHtml1 = '<button id="timepickerConfirm" style="float:right">确定</button>';   
            var strHtml2 = '<button id="timepickerCancel" style="float:right">取消</button>';
            cal.find('div.datepickerContainer').append(strHtml2).append(strHtml1);	 
            fill(cal.get(0));
            if (options.inline) {
  ...
  $.fn.extend({
	DatePickerSetConfirmed: DatePicker.setConfirmed,
